#!/usr/bin/env python3
"""Canon PG-245 Estimate-only multi-city shipping quotes.

SKUs from Plankton UnitWeight report:
  CANPG245XL, CANPG210 — UnitWeight 0.15 each.

Estimate / ShippingLabel only (never Cart Process, never $10 flat, never publish).
Gates stay OFF. Same Band-A enrichment + p75 + re-score pattern.
"""
...
SKUS = ["CANPG245XL", "CANPG210"]
PRODUCTS_ALL = Path("/workspace/firefinds/data/products_all.json")
UNITWEIGHT_REPORT = Path(
    "/workspace/firefinds/data/reports/canon_pg245_unitweight_for_estimate_20260904.json"
)
...
def main() -> int:
    ...
    stamp = _local_stamp()
    out_json = Path(
        f"/workspace/firefinds/data/reports/canon_pg245_estimate_quotes_{stamp}.json"
    )
    out_md = Path(
        f"/workspace/firefinds/data/reports/canon_pg245_estimate_quotes_{stamp}.md"
    )
    ...
    lines = [
        f"# Canon PG-245 Estimate quotes `{stamp}`",
        "",
    ...
    if listable_n == 0:
        lines.append(
            "**Neither SKU becomes listable** — ship may resolve but margin/MAP still kills "
            "(expected)."
        )
        lines.append("")
    for r in results:
